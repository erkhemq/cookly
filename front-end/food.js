...
class MainWrapper extends HTMLElement {
  ...
  connectedCallback() {
    ...
    // Listen for the custom 'like-toggled' event to update the likes
    document.addEventListener("ingredient-toggled", (event) => {
      const { ids } = event.detail;
      if (ids.length == 0) this.fetchFoods();
      else this.fetchFilteredFoods(ids);
    });
  }

  async fetchFoods() {
    try {
      const response = await fetch("/api/foods", { method: "GET" });
  ...
  async fetchFilteredFoods(ingredients) {
    try {
      const queryString = ingredients.map((id) => `${id}`).join(",");
      const response = await fetch(
        `/api/foods/ingredients?ids=${queryString}`,
        {
          method: "GET",
        }
      );
